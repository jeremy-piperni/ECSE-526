...
public class DC4Server {
    ...
    private static class ClientThread extends Thread {
		...
		// if (s < 1 || s > 3) {
		// 	throw new Exception(s + " is not a valid number of squares the piece can move. Needs to be between 1 and 3.");
		// }

		
		op = inputLine.charAt(0);
		if (op!='D' && op!='L' && op!='R') {
		    throw new Exception("operator must be one of D, L, R");
		}

		if ((op == 'D' && inputLine.length() != 3) || (op != 'D' && inputLine.length() != 5)) {
		    throw new Exception("incorrect number of parameters for operator " + op);
		}
		
                s = inputLine.charAt(1);
		if (s!=' ') {
		    throw new Exception("operator and column value must be separated by a space");
		}

                x = inputLine.charAt(2)-48;
		if (x < 1 || x > 8) {
		    throw new Exception(x + " is not a valid column value. Needs to be between 1 and 8.");
		}
                if (op != 'D') {
		    s = inputLine.charAt(3);
		    if (s!=' ') {
			throw new Exception("column and row values must be separated by a space");
		    }
                    y = inputLine.charAt(4)-48;
		    if (y < 1 || y > 8) {
			throw new Exception(y + " is not a valid row value. Needs to be between 1 and 8.");
		    }
		}
